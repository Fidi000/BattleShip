...
function printboard()//print board with num values
{
    for(var i = 0;i<10;i++){
        for(var j = 0;j<10;j++){
...
function place(){placed = !placed;}//determines if ship placed
function flip(){horizontal = !horizontal;}//determines orientation
function set_length(len){global_shiplength=len}//set length of each ship
function returnval(id) {//these function are onclick when placing ship
    place_ship(id);
    remove_clickable();
    placed = true;
}
function place_ship(coords)//places the ship on the coords outlined by outlining function, and determines if overlapping preventing placement
{
    if (!amountIsZero()){
        if(!placed){
...
function getColor()//returns color of next ship for distinction
{
...
function get_shipvalue()//returns unique value for each ship placed
{
    return shipValue++;
...
function check_overlap(len, flipped, row, col)//checks for overlap on click of ship 
{
    var overlap = false;
    var flippedcol = col;
    var normalcol = col;
...
function make_clickable()//adds functions to board after pressing place 
{
...
function show_outline(id)//show outline of possible ship placement, no outline when overlapping
{
    if (!amountIsZero()){
...
function remove_outline(id){//removes outline 
    if (!amountIsZero()){
        if(!placed){
        var shiplength = global_shiplength;
        var row = parseInt(id.charAt(0));
...
function openMenu() {//opens ship menu on left side
...
function closeMenu() {//closes ship menu on left side
    
    document.getElementById("myshipSelectMenu").style.width = "0";
    return false;
}

function amountIsZero(){//determines the number of ships remaining
...
function reduceAmount(){//reduces number of current ship being placed
...
function getShipType(id){//gets the properties of the current ship being placed, eg.(length)
    shipType = id;
}
function displayOverlapAlert(text, color){//displayer overlap with text passed into function
...
function hideOverlapAlert(){//removes overlap box
